...
Let's consider an example where we are interested in the proportion of people in the United States are multi-lingual. We create a prior distribution of $\theta$~Beta(a=2,b=8), suggesting that 20% of US residents are multi-lingual. The distribution would look like this:

![Figure](https://raw.githubusercontent.com/kbmoore02/my386blog/main/assets/images/prior.jpg)

After we collected 30 samples, 7 people were multi-lingual (successes) and 23 people were not (failures). Creating the posterior distribution for beta-binomial is very simple. It follows this formula: Beta(a* = a + successes, b* = b + failures). So for this example, the posterior distribution is Beta(9, 31):
...
```math
qbeta(c(0.025,0.975), 9, 31)
```

There is a 95% probability that the true proportion of multi-lingual people amongst all people in the US is between 0.11 and 0.36. 
